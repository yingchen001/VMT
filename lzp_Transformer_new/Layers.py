...
class VMultiHeadAttention(nn.Module):

    def __init__(self, h, d_model, p):
        super(VMultiHeadAttention, self).__init__()      
        self.h = h
        self.d = d_model
        self.d_head = d_model//h
        self.fc_query = nn.Linear(d_model, h*self.d_head, bias=False)
        ...
        self.fc_value = nn.Linear(d_model, h*self.d_head, bias=False)
        # for video
        self.fc_vkey = nn.Linear(d_model, h*self.d_head, bias=False)
        self.fc_vvalue = nn.Linear(d_model, h*self.d_head, bias=False)
        ...
        # for merge
        self.merge_fc = nn.Linear(1024, 512)
    ...
    def forward(self, query, key, value, mask, vkey, vvalue):
        b, len_query = query.size(0), query.size(1)
        len_key = key.size(1)
        ...
        proj_value = self.fc_value(value)       # batch_size x len_key x h*d_head
        # for video 
        proj_vkey = self.fc_vkey(vkey)
        proj_vvalue = self.fc_vvalue(vvalue)

        # prepare the shape for applying softmax
        proj_query = self._prepare_proj(proj_query)  # batch_size*h x len_query x d_head
        proj_key = self._prepare_proj(key)           # batch_size*h x len_key x d_head
        proj_value = self._prepare_proj(value)       # batch_size*h x len_key x d_head
        proj_vkey = self._prepare_proj(vkey)           # batch_size*h x len_key x d_head
        proj_vvalue = self._prepare_proj(vvalue) 
        ...
        # atttention for video
        vattens = torch.bmm(proj_query, proj_vkey.transpose(1,2))  # batch_size*h x len_query x len_key
        vattens = vattens / math.sqrt(self.d_head) 
        vattens = vattens.view(b, self.h, len_query, len_key) 
        # no mask for video for now
        # vattens = vattens.masked_fill_(Variable(mask.unsqueeze(1)), -float('inf'))
        vattens = self.sm(vattens.view(-1, len_key))
        # return mean attention from all heads as coverage 
        vcoverage = torch.mean(vattens.view(b, self.h, len_query, len_key), dim=1)
        vattens = self.attn_dropout(vattens)
        vattens = vattens.view(b*self.h, len_query, len_key)
        # apply vattens on value
        vout = torch.bmm(vattens, proj_vvalue)      # batch_size*h x len_query x d_head
        vout = vout.view(b, self.h, len_query, self.d_head).transpose(1,2).contiguous() 
        vout = self.fc_concat(vout.view(b, len_query, self.h*self.d_head))
        vout = self.layernorm(query + self.dropout(vout)) 
        # print('out', out.shape)
        # print('vout', vout.shape)
        # merge text output and video output and keep the same shape
        cat = torch.cat([out, vout], dim=2)
        final_out = self.merge_fc(cat)
        # print('cat', cat.shape)
        # print('final_out', final_out.shape)
        return final_out, coverage
...
class VEncoderLayer(nn.Module):
    def __init__(self, h, d_model, p, d_ff):
        super(VEncoderLayer, self).__init__()
        self.multihead = VMultiHeadAttention(h, d_model, p) # video mutli head
        self.feedforward = FeedForward(d_model, d_ff, p)
    
    def forward(self, query, key, value, mask, vkey, vvalue):
        out, _ = self.multihead(query, key, value, mask, vkey, vvalue) # add video
        out = self.feedforward(out)
        return out
