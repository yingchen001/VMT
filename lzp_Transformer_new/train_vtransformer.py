...
def trainEpoch(epoch, model, criterion, dataloader, optim, print_batch=250):
    model.train()
...
def test_nltk(model, dataloader, src_tokenizor, tgt_tokenizor, max_steps=None, print_every=-1):
    ...
    translator = VTranslator(model, 'no-bpe', beam_size=beam_size, alpha=alpha, beta=beta, max_len=40)
    ...
    for i, (srccap, tgtcap, video, caplen_src, caplen_tgt, srcrefs, tgtrefs) in enumerate(dataloader):
        ...
        try:
            ...
            hypo = pred_list[1:-1]
            if tgt_tokenizor.language=='zh' and tgt_tokenizor.use_jieba:
                hypo = tgt_tokenizor.convert_to_char_split_list(hypo)[1]
                ref = tgt_tokenizor.convert_to_char_split_list(ref)[1]
            if len(hypo) >5: # filter those too short, probably due to error
            ...
            # pass
    if tgt_tokenizor.use_jieba:
        print('\nref', refs[0])
        print('hypo', hypotheses[0])
...
if __name__ == "__main__":
    ...
    run_type = 'transformer_video_jieba' # for checkpoint saving
    num_epochs = 100
    num_layer = 4
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if (epoch % test_every == 0 and epoch > 0):
            ...
            try:
                ...
                corpbleu, sentbleu = test_nltk(model, test_loader, tok_src, tok_tgt, max_steps=1000)
                if corpbleu > best_bleu:
                    best_bleu = corpbleu
                print('BLEU score at epoch {} : {}, {}. best BLEU: {}'.format(epoch+1, corpbleu, sentbleu, best_bleu))
            except Exception as e:
                print('unable to get bleu due to: ', e)
        elif epoch == num_epochs - 1:
            print('running final testing...')
            try:
                corpbleu, sentbleu = test_nltk(model, test_loader, tok_src, tok_tgt, max_steps=None)
                print('Final BLEU score at epoch {} : {}, {}'.format(epoch+1, corpbleu, sentbleu))
            except Exception as e:
                print('unable to get bleu due to: ', e)
