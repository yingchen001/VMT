...
class Tokenizer(object):
    ...
    def __init__(self, language, vocab=None, encoding_length=30, zh_tok='jieba'):
    ...
    def decode_sentence(self, encoding, filter_EOS=False):
        sentence = []
        for ix in encoding:
            if ix == self.word_to_index['<PAD>'] or (filter_EOS and ix==self.word_to_index['<EOS>']):
                break
            else:
        ...
        if self.language=='zh':
            output = "".join(sentence) # remove space
            spaced =  " ".join(output) # add space between each char
            if '< U N K >' in spaced:
                spaced = spaced.replace('< U N K >','<UNK>')
            return spaced
        else:
            return " ".join(sentence) # unreverse before output


def build_vocab(data_dir, language, min_count=5, start_vocab=base_vocab, zh_tok='jieba'):
    ''' Build a vocab, starting with base vocab containing a few useful tokens. '''
    assert language in ['en', 'zh']
    count = Counter()
    t = Tokenizer(language, zh_tok=zh_tok)

    with open(data_dir+'vatex_training_v1.0.json', 'r') as file: # build vocab only based on training data
        data = json.load(file)
