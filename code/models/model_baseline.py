...
class Encoder_NMT(nn.Module):
    def __init__(self, vocab_size, embed_size, hidden_size,
        ...
        self.embed_size = embed_size
        self.src_embed = nn.Embedding(vocab_size, embed_size)
        self.src_encoder = nn.LSTM(input_size=embed_size, hidden_size=hidden_size, num_layers=n_layers,
                                     dropout=dropout, batch_first=True, bidirectional=True)
        self.dropout = nn.Dropout(dropout, inplace=True)
...
class Decoder_NMT(nn.Module):
    def __init__(self, embed_size, hidden_size, vocab_size,
        ...
        self.embed = nn.Embedding(vocab_size, embed_size)
        self.dropout = nn.Dropout(dropout, inplace=True)
        self.src_attention = SoftDotAttention(embed_size*2, hidden_size)
    ...
    def onestep(self, input, last_hidden, src_out, src_mask):
        '''
        input: (B,)
        '''
        # Get the embedding of the current input word (last output word)
        ...
        src_ctx, src_attn = self.src_attention(src_out, last_hidden[0][0], mask=src_mask) # src_ctx: (mb, 1, dim) attn: (mb, 1, seqlen)
        # Combine embedded input word and attended context, run through RNN
        rnn_input = torch.cat([embedded, src_ctx], 2) # (mb, 1, input_size)

        output, hidden = self.decoder(rnn_input, last_hidden) 
        output = output.squeeze(1)  # (B, 1, N) -> (B,N)
        output = self.fc(output)
        return output, hidden, src_attn

    def forward(self, src, trg, init_hidden, src_out, max_len, teacher_forcing_ratio):
        batch_size = trg.size(0)
        src_mask = (src == 0) # mask paddings.
        ...
        output = trg.data[:, 0] # <sos>
        for t in range(1, max_len):
            output, hidden, attn_weights = self.onestep(output, hidden, src_out, src_mask) # (mb, vocab) (1, mb, N) (mb, 1, seqlen)
            outputs[:, t, :] = output
            is_teacher = random.random() < teacher_forcing_ratio
            top1 = output.data.max(1)[1]
            output = (trg.data[:, t] if is_teacher else top1).cuda() # output should be indices to feed into nn.embedding at next step
    ...
    def inference(self, src, trg, init_hidden, src_out, max_len, teacher_forcing_ratio=0):
        '''
        Greedy decoding
        '''
        batch_size = trg.size(0)
    ...
    def beam_decoding(self, src, init_hidden, src_out, max_len, beam_size=5):
        batch_size = src.size(0)
        src_mask = (src == 0) # mask padding
        hidden = (init_hidden[0][:self.n_layers].contiguous(), init_hidden[1][:self.n_layers].contiguous())
        ...
        for i in range(batch_size):
            ...
            output, hidden_i, attn_weights = self.onestep(output, hidden_i, src_out_i, src_mask_i) # (mb, vocab) (1, mb, N) (mb, 1, seqlen)
            log_probs = F.log_softmax(output, dim=1)
            log_probs[:, -1] = log_probs[:, -1] - 1000
            neg_log_probs = -log_probs
            ...
            for j in range(max_len):
                ...
                output = torch.from_numpy(outputs).cuda()
                hidden_i = self.from_numpy(new_state_d)
                output, hidden_i, attn_weights = self.onestep(output, hidden_i, src_out_i, src_mask_i)
                log_probs = F.log_softmax(output, dim=1)
