...
class Encoder(nn.Module):
    def __init__(self, vocab_size, embed_size, aud_size, hidden_size,
        ...
        self.frame_embed = nn.Linear(3072, self.embed_size*2) # concat resnext and i3d
        self.video_encoder = nn.LSTM(input_size=embed_size*2, hidden_size=hidden_size // 2, num_layers=n_layers,
                                     dropout=dropout, batch_first=True, bidirectional=True)

        self.audio_embed = nn.Linear(aud_size, aud_size)
        self.audio_encoder = nn.LSTM(input_size=aud_size, hidden_size=aud_size // 2, num_layers=n_layers,
                                     dropout=dropout, batch_first=True, bidirectional=True)

        self.dropout = nn.Dropout(dropout, inplace=True)

    def forward(self, src, vid, aud, src_hidden=None, vid_hidden=None, aud_hidden=None):
        ...
        aud_embedded = self.audio_embed(aud)
        aud_out, aud_states = self.audio_encoder(aud_embedded, aud_hidden)

        aud_h = aud_states[0].permute(1, 0, 2).contiguous().view(
            batch_size, 2, -1).permute(1, 0, 2)
        aud_c = aud_states[1].permute(1, 0, 2).contiguous().view(
            batch_size, 2, -1).permute(1, 0, 2)

        init_h = torch.cat((src_h, vid_h, aud_h), 2)
        init_c = torch.cat((src_c, vid_c, aud_c), 2)

        return src_out, (init_h, init_c), (vid_out, aud_out)

class Encoder_AUD(nn.Module):
    def __init__(self, vocab_size, embed_size, aud_size, hidden_size,
                 n_layers=2, dropout=0.5):
        super(Encoder_AUD, self).__init__()
        self.hidden_size = hidden_size
        self.embed_size = embed_size
        self.src_embed = nn.Embedding(vocab_size, embed_size)
        self.src_encoder = nn.LSTM(input_size=embed_size, hidden_size=hidden_size // 2, num_layers=n_layers,
        ...
        self.frame_embed = nn.Linear(1024, self.embed_size) # concat resnext and i3d
...
class Decoder(nn.Module):
    def __init__(self, embed_size, aud_size, hidden_size, vocab_size,
                 n_layers=2, dropout=0.5):
        ...
        self.aud_attention = SoftDotAttention(aud_size, hidden_size)

        self.decoder = nn.LSTM(embed_size*3 + aud_size, hidden_size,
    ...
    def onestep(self, input, last_hidden, src_out, v_outs, src_mask):
        '''
        input: (B,)
        '''
        # v_outs = (vid_out, aud_out)
        # Get the embedding of the current input word (last output word)
        embedded = self.embed(input).unsqueeze(1)  # (B,1, N)
        embedded = self.dropout(embedded)
        # Calculate attention weights and apply to encoder outputs
        # print(src_out.shape, last_hidden[0][0].shape)
        src_ctx, src_attn = self.src_attention(src_out, last_hidden[0][0], mask=src_mask) # src_ctx: (mb, 1, dim) attn: (mb, 1, seqlen)
        vid_ctx, vid_attn = self.vid_attention(v_outs[0], last_hidden[0][0])
        aud_ctx, aud_attn = self.aud_attention(v_outs[1], last_hidden[0][0])
        # Combine embedded input word and attended context, run through RNN
        rnn_input = torch.cat([embedded, src_ctx, vid_ctx, aud_ctx], 2) # (mb, 1, input_size)
        # print(rnn_input.is_contiguous(), last_hidden[0].is_contiguous(), last_hidden[1].is_contiguous())
        output, hidden = self.decoder(rnn_input, last_hidden) 
        output = output.squeeze(1)  # (B, 1, N) -> (B,N)
        output = self.fc(output)
        return output, hidden, (src_attn, vid_attn, aud_attn)

    def forward(self, src, trg, init_hidden, src_out, v_outs, max_len, teacher_forcing_ratio):
        ...
        for t in range(1, max_len):
            output, hidden, attn_weights = self.onestep(output, hidden, src_out, v_outs, src_mask) # (mb, vocab) (1, mb, N) (mb, 1, seqlen)
            outputs[:, t, :] = output
    ...
    def inference(self, src, trg, init_hidden, src_out, v_outs, max_len, teacher_forcing_ratio=0):
        '''
        Greedy decoding
        '''
        batch_size = trg.size(0)
    ...
    def beam_decoding(self, src, init_hidden, src_out, v_outs, max_len, beam_size=5):
        batch_size = src.size(0)
        ...
        for i in range(batch_size):
            ...
            vid_out_i = v_outs[0][i].unsqueeze(0).expand(beam_size, v_outs[0].size(1), v_outs[0].size(2)).contiguous()
            aud_out_i = v_outs[1][i].unsqueeze(0).expand(beam_size, v_outs[1].size(1), v_outs[1].size(2)).contiguous()
            v_outs_i = (vid_out_i, aud_out_i)
            ...
            output, hidden_i, attn_weights = self.onestep(output, hidden_i, src_out_i, v_outs_i, src_mask_i) # (mb, vocab) (1, mb, N) (mb, 1, seqlen)
            ...
            for j in range(max_len):
                ...
                output, hidden_i, attn_weights = self.onestep(output, hidden_i, src_out_i, v_outs_i, src_mask_i)
                log_probs = F.log_softmax(output, dim=1)
...
class Encoder_RES(nn.Module):
    def __init__(self, vocab_size, embed_size, hidden_size,
        ...
        self.frame_embed = nn.Linear(3072, self.embed_size*2)
        self.video_encoder = nn.LSTM(input_size=embed_size*2, hidden_size=hidden_size // 2, num_layers=n_layers,
                                     dropout=dropout, batch_first=True, bidirectional=True)
