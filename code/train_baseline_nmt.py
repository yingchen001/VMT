...
from models.model_baseline import Encoder_NMT, Decoder_NMT
...
def main(args):
    ...
    if args.model_type == 's2s':
        encoder = Encoder_NMT(vocab_size=len(vocab_src), embed_size=args.wordembed_dim, hidden_size=args.enc_hid_size).cuda()
        decoder = Decoder_NMT(embed_size=args.wordembed_dim, hidden_size=args.dec_hid_size, vocab_size=len(vocab_tgt)).cuda()
...
def train(train_loader, encoder, decoder, criterion, enc_optimizer, dec_optimizer, epoch):
    ...
    for cnt, (srccap, tgtcap, video, caplen_src, caplen_tgt, srcrefs, tgtrefs) in enumerate(train_loader, 1):
        ...
        src_out, init_hidden = encoder(srccap) # fea: decoder input from encoder, should be of size (mb, encout_dim) = (mb, decoder_dim)
        scores = decoder(srccap, tgtcap, init_hidden, src_out, args.MAX_INPUT_LENGTH, teacher_forcing_ratio=args.teacher_ratio)
...
def validate(val_loader, encoder, decoder, criterion, tok_tgt):
    ...
    with torch.no_grad():
        ...
        for cnt, (srccap, tgtcap, video, caplen_src, caplen_tgt, srcrefs, tgtrefs) in enumerate(val_loader, 1):
            ...
            # Forward prop.
            src_out, init_hidden = encoder(srccap) # fea: decoder input from encoder, should be of size (mb, encout_dim) = (mb, decoder_dim)
            scores, pred_lengths = decoder.inference(srccap, tgtcap, init_hidden, src_out, args.MAX_INPUT_LENGTH)
...
def eval(test_loader, encoder, decoder, cp_file, tok_tgt, result_path):
    ...
    references = list()  # references (ground truth)
    hypotheses = list()  # hypotheses (predictions)

    with torch.no_grad():
        # Batches
        for cnt, (srccap, tgtcap, video, audio, caplen_src, caplen_tgt, srcrefs, tgtrefs, sent_id) in enumerate(tqdm(test_loader)):
            srccap, tgtcap, caplen_src, caplen_tgt = srccap.cuda(), tgtcap.cuda(), caplen_src.cuda(), caplen_tgt.cuda()
            video = video.cuda()
            # Forward prop.
            src_out, init_hidden = encoder(srccap) # fea: decoder input from encoder, should be of size (mb, encout_dim) = (mb, decoder_dim)
            preds, pred_lengths = decoder.beam_decoding(srccap, init_hidden, src_out, args.MAX_INPUT_LENGTH, beam_size=5)
    ...
    for i, (r, h) in enumerate(zip(references, hypotheses), 1):
        sentbleu += sacrebleu.sentence_bleu([h], [[r]]).score
    sentbleu /= i
    print('Bema Search BLEU-4: ',corpbleu, sentbleu)
    # Save results
