...
def trainEpoch(epoch, model, criterion, dataloader, optim, print_batch=1000):
    model.train()
...
def test_nltk(model, dataloader, src_tokenizor, tgt_tokenizor, max_steps=None, print_every=-1):
    ...
    translator = Translator(model, 'no-bpe', beam_size=beam_size, alpha=alpha, beta=beta, max_len=40)
...
if __name__ == "__main__":
    ...
    run_type = args.run_type # for checkpoint saving
    num_epochs = 100
    num_layer = 4
        ...
        if (epoch % test_every == 0 and epoch > 0):
            ...
            try:
                ...
                if corpbleu > best_bleu:
                    best_bleu = corpbleu
                print('BLEU score at epoch {} : {}, {}, best BLEU: {}'.format(epoch, corpbleu, sentbleu, best_bleu))
            except Exception as e:
            ...
            try:
                ...
                print('Final BLEU score at epoch {} : {}, {}'.format(epoch, corpbleu, sentbleu))
            except Exception as e:
                print('unable to get bleu due to: ', e)
